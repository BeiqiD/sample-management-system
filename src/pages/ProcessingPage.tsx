...
function matchesFilter(sample: SampleSummary, filter: ProcessingFilter) {
  ...
  if (filter === "active") return sample.status === "active" && (sample.currentRecipeStatus === "active" || !sample.currentRecipeStatus);
  return sample.currentRecipeStatus === filter;
}
...
export function ProcessingPage() {
  ...
  return <div className="page processing-page">
    ...
    <div className="processing-controls">
      ...
      <label className="search-box compact-search"><span>Search</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Code, title, recipe, or location" /></label>
    </div>
    ...
    {loading ? <p className="muted">Loading…</p> : visibleSamples.length ? <div className="processing-list">
      ...
      {query ? "Try another code, title, recipe, or location." : filter === "active" ? "Active samples without a workflow will also appear here, ready to start." : "Choose another status to inspect other runs."}
    </EmptyState>}
  </div>;
}
